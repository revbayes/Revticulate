...
initRev("D://RevBayes_Win_1.1.0/RevBayes_Win_1.1.0/rb.exe")
knitRev()
```
...
#Right now, the best way to do this is to use phytools or ape to read in a RevBayes tree and plot to screen. Coercion of tree objects from RevBayes to R is still an area under active development.# 

One thing researchers are often interested in doing is making an object in Rev and then viewing it in R. Objects created with Rev code are, by default, String representations of actual Rev objects.
These objects can be used in R chunks with the function getRevObj().
...
Rev objects can be converted into similar R objects with the function coerceRev(). Rev Tree objects are coerced into phylo objects from the ape package.

```{r}
myTree <- getRevObj("phylogeny")
coerceRev(myTree)
```

This process is can be simplified by setting the argument coerce to TRUE in the getRevObj() function.

```{r}
getRevObj("phylogeny", coerce = T)
```

Now lets clear the Rev environment:
...
Revticulate functions use a forced timeout to get output from rb. This may result in seemingly out-of-place timeout warnings.
