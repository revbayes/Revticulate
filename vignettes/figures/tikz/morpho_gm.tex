...
\begin{tikzpicture}
\node[constnode] (lambda) at (1,1.6) {\large $\delta_{\beta}$};
\node[snode] (beta) at ($(lambda)+(1.5,0)$) {\Large $\beta_{\beta}$};
\node[constnode] (delta_alpha) at (1,0.4) {\large $\delta_{\alpha}$};
\node[snode] (alpha) at ($(delta_alpha)+(1.5,0)$) {\Large $\alpha_{\beta}$};
\node[detnode] (pi) at ($(beta)+(2,-0.6)$) {\LARGE $\pi_i$};
...
\node (eb) at ($(alpha)+(0,-0.7)$) [centered]{\scriptsize exponential};
