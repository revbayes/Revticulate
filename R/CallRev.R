#'Calls function from rb and returns object in R
#'
#' @param ... Command to be passed into rb
#' @param coerce If TRUE, attempts to coerce output into R object. if FALSE, returns raw string output from rb.
#' @param path Path to rb. If InitRev has been called, defaults to revenv.RevPath.
#' @param viewCode If TRUE, uses stringr::str_view_all function to view raw input and output sent to rb
#'
#' @return out: Output object from rb. If coerce = TRUE, out will be an R equilvalent of rev object. If FALSE, out will be string representation of the object.
#'
#' @export
#'
#'
...
if(any(stringr::str_detect(argu, "<-|=|~|:=")) == TRUE){
  RevR::RevDefine(argu[which(stringr::str_detect(argu, "<-|=|~|:="))])
}
...
ret <- unlist(lapply(argu, RevR::repRevObjects))
