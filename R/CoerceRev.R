...
#' or even phylo tree) to convert the string into.
#'
#' @param ... out
#'
#' @return out convert output of the RevBayes output string.
#'
#' @examples
#' CoerceRev("[1, 2, 3, 4]")
#'
...
CoerceRev <- function(out, notCoerce = F){
  ...
  #convert simple vectors, strings, and numerics, as well as single line phylogenies
  if(length(out) == 1){


    if(test_phylo(out) == TRUE){
      out <- convert_phylo(out)
      return(out)
    }
  ...
  if(test_matrix(out) == TRUE){convert = "array"}
